...
// EVERYTHING IS FROM AI
// prompt:
/*

write rust program that:
is emoji picker for TUI
executable ./emoji-pick
is controlled by Arrow keys (add support for hjkl but make it as --vim-motions option)
add --help message.
when the selected emoji is the same (no corsor is moved) then show popup/ like how title works for html that shows the original text that is for example :happy_face:.

the cols should be 9 and dynamic height, note that not everything will fit in single screen so it needs to have scroll.

it should have different tabs like :
:grinning:	Smileys & Emotions
:wave::tone_light:	People
...
(Pasted Test 1) is all of the text emojies
(Pasted Text 2) is mapping from text emoji to unicode
depending on option --unicode or --text output that to the terminal on enter clicking.
by default ouput the unicode.
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let output_text = args.text || !args.unicode;
