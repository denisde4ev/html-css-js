...
/*globals Window,HTMLElement,HTMLDocument,HTMLCollection,NodeList,MutationObserver */
/*exported Arrive*/
...
// fork of https://github.com/uzairfarooq/arrive/blob/master/src/arive.js
// forked by denisde4ev
// fork version: v0.1
// changelog:
// v0.1 mostly syntax and preferences + remod jQuery and rm bind to prototype
...
var Arrive = (function(window, undefined) {
	...
	var utils = (function() {
		var matches = HTMLElement.prototype.matches; // consider for old browsers: webkit*, moz*, ms*
		...
		return {
			...
			callCallbacks: function(callbacksToBeCalled, registrationData, mutationEvents) {
				...
				for (let i = 0, cb; (cb = callbacksToBeCalled[i]); i++) {
					if (cb && cb.callback) {
				...
				if (registrationData && registrationData.callback && mutationEvents) {
					mutationEvents.addTimeoutHandler(
						registrationData.target,
						registrationData.selector,
						registrationData.callback,
			...
			checkChildNodesRecursively: function(nodes, registrationData, matchFunc, callbacksToBeCalled) {
				...
				for (let i=0, node; (node = nodes[i]); i++) {
					...
					if (node.childNodes.length > 0) {
						utils.checkChildNodesRecursively(
							node.childNodes,
							registrationData,
							matchFunc,
							callbacksToBeCalled,
	...
	var EventsBucket = (function() {
		...
		EventsBucket.prototype.addEvent = function(
			target,
			selector,
			options,
			callback,
		...
		EventsBucket.prototype.removeEvent = function(compareFunction) {
			for (
				var i=this._eventsBucket.length - 1, registeredEvent;
				(registeredEvent = this._eventsBucket[i]);
				i--
			) {
		...
		this.unbindEventWithSelectorOrCallback = function(selector) {
			...
			if (typeof selector === "function") {
				compareFunction = function(eventObj) {
					for (let i = 0; i < elements.length; i++) {
						if (
							(this === undefined || eventObj.target === elements[i])
							&&
