...

{
	...
	outputEl.parentElement.childNodes.forEach(e => { if (e.nodeType === Node.TEXT_NODE) e.remove() });
...
document.addEventListener('submit', e => {
	...
	var req_body = (
		...
		*/
		...
		//	[...form_data]
		//:
		// todo: needs to be obj, and duplicated keys to have arry of all values of the same key */
	...
	fetch(form_url, {
		...
		res.json().then(function(obj) {
			...
			var fnStr = form.getAttribute('data-formSubmit-done');
			Function(`
				form,
				data,
			`, fnStr ).call( /*this=*/form,
				form,
				obj
			);
	...
	// todo: add form loading class + status(aria-busy) for form and buttons
